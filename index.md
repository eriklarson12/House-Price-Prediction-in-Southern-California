...
**Linear Regression (Supervised) Model/Algorithm**: (`sklearn.linear_model.LinearRegression`)  
...
This summarizes the model's performance by providing training and test set metrics. The metrics point to a model that consistently underfits the structure of Southern California housing prices. Errors remain extremely high with an RMSE above $300k which is over 40% of the typical home price. The near-identical train and test performance shows that the model is not struggling with variance but with limited capacity to capture meaningful relationships. Cross-validation reinforces this, with low spread across folds but equally weak results, indicating stable yet shallow learning. Ridge regularization smooths out patterns that, in housing markets, tend to be nonlinear and heavily driven by location-specific factors, so important signals end up muted. Overall, the output reflects a model that captures broad averages but misses the finer-grained dynamics required for practical price prediction in this domain.


![A screenshot of ridge regression graph results](images/ridge_imgs.png)

The scatter plots make the model’s limitations even harder to ignore. Predictions cluster tightly in a narrow band, especially for higher-priced homes, which shows that the model collapses a wide range of actual values into similar predicted outputs. Instead of tracking the upward trajectory of real prices, the estimates flatten out toward the mean, a classic sign of heavy regularization suppressing hidden meaningful variation. The wide vertical spread at almost every actual price point reflects substantial error, and the consistent gap between the cloud of points and the perfect-prediction line shows that the model understates expensive homes while overstating cheaper ones. The nearly identical R² values for training and test data, combined with the similar scatter patterns, confirm that this isn’t an overfitting issue. The model simply doesn’t capture enough of the underlying structure to produce accurate or nuanced predictions.
...
### Quantitative Metrics for Ridge Regression

**Root Mean Squared Error (RMSE)**: The Ridge Regression model produced a test RMSE of $309,292.71, meaning the average prediction deviates from the actual sale price by about $309K. This corresponds to 43.74% of the average home price in the dataset of $707,084.58. The training RMSE was similar at $301,803.21 (about 42.98% of average price), indicating that the model is able to generalize reasonably well and does not suffer from any major overfitting. Ridge’s RMSE performance is nearly identical to standard Linear Regression which suggests that regularization did not strongly affect model error.

**Mean Absolute Error (MAE)**: The model had a test MAE of $228,132.62, which indicates the typical absolute prediction error is around $228K, also aligning closely with the Linear Regression MAE. The training MAE was $222,578.16, again showing a small difference between training and test performance. This consistency suggests Ridge Regression slightly stabilizes errors without significantly changing the model’s underlying structure.

**R² Score**: Ridge Regression achieved an R² of 0.3510 on the test set which means it explains about 35% of the variance in home prices. This is nearly identical to the Linear Regression R² of 0.3509, reflecting that while Ridge regularization improves model stability and reduces coefficient sensitivity, it does not substantially increase predictive power using the current features.
